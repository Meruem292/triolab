...
if (isset($_POST['archive_patient'])) {
    $patientIdDelete = $_POST['patientIdDelete'];
    $archive = '1';

    // Update the patient archive status
    $archiveQuery = $pdo->prepare("UPDATE patient SET is_archive = :archive WHERE id = :id");
    $archiveQuery->bindParam(':archive', $archive);
    $archiveQuery->bindParam(':id', $patientIdDelete);

    if ($archiveQuery->execute()) {
        $_SESSION['message'] = "Patient archived successfully.";
        $_SESSION['status'] = "success";
    } else {
        $_SESSION['message'] = "Error archiving patient.";
