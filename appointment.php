...
$selectService = $pdo->query("SELECT * FROM services WHERE id = '$serviceId'");
...
if (isset($_POST['add_appointment'])) {
    ...
    $sql_select = $pdo->prepare("SELECT * FROM appointment WHERE appointment_date = :selectedDate AND appointment_schedule = :selectedSchedule AND appointment_time = :selectedTime");
    ...
    if ($sql_select->rowCount() > 0) {
        $_SESSION['message'] = "Appointment conflict: An appointment already exists at the selected date, time, and schedule.";
        $_SESSION['status'] = "error";
    } else {
        $sql_update_user = $pdo->prepare("UPDATE patient SET firstname = :firstname, lastname = :lastname, email = :email, dob = :birthdate, birthplace = :birthplace, contact = :contactNumber, province = :province, city = :city, barangay = :barangay, street = :street WHERE id = :user_id");
        $sql_update_user->execute(array(
            ':firstname' => $firstname,
            ':lastname' => $lastname,
            ':email' => $email,
            ':birthdate' => $birthdate,
        ...
        $sql_insert = $pdo->prepare("INSERT INTO appointment (service_id, appointment_date, appointment_schedule, appointment_time, patient_id, selectedPayment, medical) VALUES (:serviceID, :selectedDate, :selectedSchedule, :selectedTime, :patient_id, :selectedPayment, :medical)");
        ...
        // Deduct one slot from the appointment slot table
        $sql_update = $pdo->prepare("UPDATE appointment_slots SET slot = slot - :slot WHERE schedule = :selectedSchedule AND date = :selectedDate");
        ...
        // status message
        $_SESSION['message'] = "Your appointment is success. Please wait for approval.";
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" id="provinceName" class="form-control" name="province" placeholder="Barangay Name" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" id="cityName" class="form-control" name="city" placeholder="Barangay Name" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (slotsData.length > 0) {
                        ...
                        slotsData.forEach(function(slotData) {
                            ...
                            slotDiv.innerHTML = `
                    <input type="radio" name="selectedSchedule" value="${slotData['schedule']}" ${slotData['selected'] ? 'checked' : ''}>
