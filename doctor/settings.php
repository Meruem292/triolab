                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <?php
                                            // Assuming the getDoctor function is fetching the doctor's data from the database.
                                            $queryDoctor = "SELECT * FROM doctor WHERE employee_id = :employee_id";
                                            $stmtDoctor = $pdo->prepare($queryDoctor);
                                            $stmtDoctor->bindParam(':employee_id', $admin_id);
                                            $stmtDoctor->execute();
                                            $doctor = $stmtDoctor->fetch(PDO::FETCH_ASSOC);
                                            ?>

                                            <h2>Update Profile</h2>
                                            <form action="../doctor/api.php" method="POST" enctype="multipart/form-data">
                                                <div class="mb-3">
                                                    <label for="employee_id" class="form-label">Employee ID</label>
                                                    <input type="text" class="form-control" id="employee_id" name="employee_id" value="<?= $_SESSION['user_id'] ?>" required readonly>
                                                </div>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="username" name="username" value="<?= htmlspecialchars($doctor['username']) ?>" required>
                                                </div>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <?php if (!empty($doctor['profile_img'])): ?>
                                                        <div>
                                                            <img src="<?= htmlspecialchars($doctor['profile_img']) ?>" alt="Profile Image" class="img-thumbnail" width="100">
                                                        </div>
